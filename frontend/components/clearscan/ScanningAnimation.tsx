  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const lineIndex = Math.min(
        LOG_LINES.length - 1,
        Math.floor((p / 100) * LOG_LINES.length)
      );
      setLogIndex(lineIndex);
    }, 80);
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="w-full lg:w-96 flex flex-col rounded-xl border border-gray-800 bg-gray-900 overflow-hidden">
        ...
        <div className="flex-1 p-4 font-mono text-sm text-[#FF8C00] min-h-[200px]">
          ...
          {progress < 100 && (
            <span className="inline-block w-2 h-4 bg-[#FF8C00] animate-pulse" />
          )}
        </div>
        <div className="px-4 py-3 border-t border-gray-800">
          <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
            <div
              className="h-full bg-[#FF8C00] transition-all duration-300 rounded-full"
